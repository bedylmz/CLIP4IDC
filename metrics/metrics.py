...
def tensor_text_to_video_metrics(sim_tensor, top_k=[1, 5, 10]):
    ...
    # Permute sim_tensor so it represents a sequence of text-video similarity
    # matrices. Then obtain the double argsort to position the rank on the
    # diagonal
    stacked_sim_matrices = sim_tensor.permute(1, 0, 2)
    first_argsort = torch.argsort(
        stacked_sim_matrices,
        dim=-1,
        descending=True,
    )
    ...
    permuted_original_data = torch.flatten(
        torch.diagonal(sim_tensor, dim1=0, dim2=2),
    )
    mask = ~torch.logical_or(
        torch.isinf(
            permuted_original_data,
        ),
        torch.isnan(permuted_original_data),
    )
    valid_ranks = ranks[mask]
    # A quick dimension check validates our results, there may be other
    # correctness tests pending Such as dot product localization, but that
    # is for other time.
    # assert int(valid_ranks.shape[0]) ==  sum([len(text_dict[k])
    #                                          for k in text_dict])
    if not torch.is_tensor(valid_ranks):
        valid_ranks = torch.tensor(valid_ranks)
    results = {
        f"R{k}": float(
            torch.sum(valid_ranks < k) * 100 / len(valid_ranks),
        )
        for k in top_k
    }
