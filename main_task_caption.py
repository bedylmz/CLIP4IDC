...
from pycocoevalcap.eval import Meteor
from pycocoevalcap.eval import PTBTokenizer
from pycocoevalcap.eval import Rouge
...
class EvalCap(COCOEvalCap):
    ...
    def evaluate(self):
        ...
        # Set up scorers
        # =================================================
        print("setting up scorers...", flush=True)
        scorers = [
        ...
        for scorer, method in scorers:
            ...
            if isinstance(method, list):
                for sc, scs, m in zip(score, scores, method):
                    ...
                    print(f"{m}: {sc:0.3f}", flush=True)
            else:
                self.setEval(score, method)
                self.setImgToEvalImgs(scores, gts.keys(), method)
                print(f"{method}: {score:0.3f}", flush=True)
        self.setEvalImgs()
...
def get_args(description="CLIP4IDC on Captioning Task"):
    ...
    parser.add_argument(
        "--task_type", default="caption", type=str, help="Point the task `retrieval` or `caption` to finetune."
    )
...
def set_seed_logger(args):
    ...
    torch.cuda.set_device(0)  # Set to first GPU (since there's only one)
...
def init_device(args, local_rank):
    ...
    if args.batch_size % args.n_gpu != 0 or args.batch_size_val % args.n_gpu != 0:
        raise ValueError(
            "Invalid batch_size/batch_size_val and n_gpu parameter: "
            "{}%{} and {}%{}, should be == 0".format(
                args.batch_size,
                args.n_gpu,
                args.batch_size_val,
                args.n_gpu,
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                logger.info(
                    ...
                    "-".join(
                        [str("%.9f" % itm) for itm in sorted(list(set(optimizer.get_lr())))],
                    ),
                    float(loss),
...
def beam_decode_step(
    ...
    def predict_word(
        ...
        next_decoder_mask = torch.ones(
            next_decoder_ids.size(),
            dtype=torch.uint8,
        ).to(device)

        # dec_output = decoder(sequence_output_rpt, visual_output_rpt,
        #                      input_ids_rpt, input_mask_rpt,
        #                      video_mask_rpt, next_decoder_ids,
        #                      next_decoder_mask, shaped=True, get_logits=True)
        dec_output = decoder(
            visual_output_rpt,
            video_mask_rpt,
            next_decoder_ids,
    ...
    # Update the beam with predicted word prob information and collect
...
def beam_decode(
    ...
    _, _, sequence_output, visual_output = model.get_sequence_and_visual_output(
        input_ids,
        segment_ids,
        input_mask,
        image_pairs,
...
def greedy_decode(
    ...
    _, _, _, visual_output = model.get_sequence_and_visual_output(
        input_ids,
        segment_ids,
        input_mask,
        image_pairs,
    ...
    input_caption_ids = torch.zeros(
        visual_output.shape[0],
        device=visual_output.device,
    ).data.fill_(tokenizer.vocab["<|startoftext|>"])
    ...
    for i in range(args.max_words):
        decoder_scores = model.decoder_caption(
            visual_output,
            video_mask,
            input_caption_ids,
            decoder_mask,
...
def eval_epoch(
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_dataloader):
            ...
            for re_idx, (image_name, re_list) in enumerate(
                zip(image_names, result_list),
            ):
                # logger.info(">" * 10)
                decode_text_list = tokenizer.convert_ids_to_tokens(re_list)
                # logger.info(f"Generated tokens: {decode_text_list}")
                ...
                # logger.info("Predicted: " + str(new_decode_item))
                # logger.info(">" * 10)

    total_results = all_result_lists + all_nc_result_lists
    json.dump(
        total_results,
        open(os.path.join(args.output_dir, "hyp_ep_%s.json" % epoch), "w"),
    )
    ...
    metrics_nlg = score_generation(
        os.path.join(args.gt_dir, "C:/Users/AliCan/Desktop/clip4idc/images/coco_format.json"),
        os.path.join(args.output_dir, "hyp_ep_%s.json" % epoch),
    )

    logger.info(
        ">>>  BLEU_1: "
        "{:.4f}, BLEU_2: {:.4f}, BLEU_3: {:.4f}, BLEU_4: {:.4f}".format(
            metrics_nlg["Bleu_1"],
            metrics_nlg["Bleu_2"],
            metrics_nlg["Bleu_3"],
            metrics_nlg["Bleu_4"],
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            if args.local_rank == 0:
                ...
                if best_score <= CIDEr:
                    ...
                    early_stop = 0
                logger.info(
                    f"The best model is: {best_output_model_file}, " f"the CIDEr is: {best_score:.4f}",
                )
                if early_stop > 10:
                    break
